                    ...
                    <table class="table mb-0" id="sortable">
                        ...
                        @foreach ($investment->houses as $index => $p)
                            <tr id="recordsArray_{{ $p->id }}">
                                ...
                                <td class="option-120">
                                    <div class="btn-group">
                                        <a href="{{route('admin.developro.house.edit', $p->id)}}" class="btn action-button mr-1" data-toggle="tooltip" data-placement="top" title="Edytuj dom"><i class="fe-edit"></i></a>
                                        <form method="POST" action="{{route('admin.developro.house.destroy', $p->id)}}">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                                            <button type="submit" class="btn action-button confirm" data-toggle="tooltip" data-placement="top" title="Usuń dom" data-id="{{ $p->id }}"><i class="fe-trash-2"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12 d-flex justify-content-end">
                    <a href="{{route('admin.developro.house.create', $investment)}}" class="btn btn-primary">Dodaj dom</a>
                </div>
            </div>
        </div>
    </div>
